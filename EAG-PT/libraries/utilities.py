import platform, random, pathlib, math, inspect, datetime, zoneinfo, time, os
...
class ExLog:
    ...
    def __init__(
        ...
        if True:
            ...
            print(
                f"{ExLog.Styles.ITALICS}[{title}]\t{ExLog.GetFileInfo(stack_offset=stack_offset)}\t{TIME_STR}{ExLog.Styles.RESET}\n\t"
...
class UTILITIES_IO:
    ...
    def ReadExrImage(path: pathlib.Path) -> torch.Tensor:
        ...
        return torch.tensor(einops.rearrange(image, "h w c -> c h w"))

    def ReadExrDepthFromBlender(path: pathlib.Path) -> torch.Tensor:
        input_image = oiio.ImageInput.open(str(path))

        if not input_image:
            raise RuntimeError(f"Failed to open {path} with OpenImageIO.")
            exit(-1)

        spec = input_image.spec()
        pixels = input_image.read_image(format=oiio.BASETYPE.FLOAT)
        input_image.close()

        image = np.asarray(pixels).reshape(spec.height, spec.width, spec.nchannels)

        return torch.tensor(einops.rearrange(image, "h w c -> c h w"))

    def ReadExrNormalFromBlender(path: pathlib.Path) -> torch.Tensor:
        input_image = oiio.ImageInput.open(str(path))
    ...
    # save RGB
    def SaveExrImage(image: torch.Tensor, path: pathlib.Path) -> None:
        pathlib.Path(path).parents[0].mkdir(parents=True, exist_ok=True)

        image = image.clone().to(dtype=torch.float32).cpu().numpy()

        image = einops.rearrange(image, "c h w -> h w c")

        # padding zeros if channel == 2
        if image.shape[2] == 2:
            image = np.concatenate(
                [image, np.zeros((*image.shape[:2], 1))], axis=-1, dtype=np.float32
            )
        ...
        ExLog(f"Save image at {path}.", stack_offset=1)
...
class UTILITIES_IMAGE:
    ...
    def Psnr(
        ...
        if image.shape[0] == 3 and target.shape[0] == 3:
            # only use rgb channels for PSNR calculation
            mse = (((image - target)) ** 2).reshape((3, -1)).mean(1, keepdim=True)
            # ExLog("DEBUG", f"{mse.shape=}")
            # get mean of rgb 3 channels
            return (20 * torch.log10(1.0 / torch.sqrt(mse))).mean()
        elif image.shape[0] == 1 and target.shape[0] == 1:
            mse = (((image - target)) ** 2).reshape((1, -1)).mean(1, keepdim=True)
            return (20 * torch.log10(1.0 / torch.sqrt(mse))).mean()
        else:
            ExLog(f"{image.shape=} {target.shape=}")
            raise NotImplementedError
    ...
    LPIPS_MODEL = lpips.LPIPS(net="vgg").to(device="cuda")

    def Lpips(image: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        return UTILITIES_IMAGE.LPIPS_MODEL(image, target)

    Denoiser540x360 = mi.OptixDenoiser(input_size=(540, 360))
    Denoiser360x540 = mi.OptixDenoiser(input_size=(360, 540))

    # this denoiser is not differentiable
    def Denoise(image: torch.Tensor) -> torch.Tensor:
        if image.shape[2] == 540 and image.shape[1] == 360:
            ExLog(f"Use created denoiser.")
            denoiser = UTILITIES_IMAGE.Denoiser540x360
        elif image.shape[2] == 360 and image.shape[1] == 540:
            ExLog(f"Use created denoiser.")
            denoiser = UTILITIES_IMAGE.Denoiser360x540
        elif image.shape[2] == 1080 and image.shape[1] == 720:
            ExLog(f"Use created denoiser.")
            denoiser = UTILITIES_IMAGE.Denoiser1080x720
        elif image.shape[2] == 720 and image.shape[1] == 1080:
            ExLog(f"Use created denoiser.")
            denoiser = UTILITIES_IMAGE.Denoiser720x1080
        else:
            ExLog(f"Create new denoiser with shape {image.shape}.")
            denoiser = mi.OptixDenoiser(input_size=(image.shape[2], image.shape[1]))
        image_denoised: dr.cuda.ad.TensorXf = denoiser(
            noisy=dr.cuda.ad.TensorXf(
                einops.rearrange(
                    image.clone(),
                    "c h w -> h w c",
                )
...
class UTILITY_FLIP:
    ...
    def CalculateFlipValues(
        ...
        flipErrorMap, meanFLIPError, parameters = flip_evaluator.evaluate(
            einops.rearrange(image_reference, "c h w -> h w c").cpu().numpy(),
            einops.rearrange(image_test, "c h w -> h w c").cpu().numpy(),
            "HDR",
        )

        return (
            torch.from_numpy(einops.rearrange(flipErrorMap, "h w c -> c h w")),
            meanFLIPError,
        )


class UTILITIES_PYTORCH:
...
class UTILITIES_COLOUR:
    ...
    # linear value 1.0 == 100 nit == PQ value 0.5
    # linear value 10.0 == 1000 nit == PQ value 0.75
    # linear value 100.0 == 10000 nit == PQ value 1.0

    def LinearToPq(linear_values: torch.Tensor):
        Y_m1 = (linear_values.clamp(min=1e-6) / 100.0) ** UTILITIES_COLOUR.m1
        return torch.pow(
            (
                (UTILITIES_COLOUR.c1 + UTILITIES_COLOUR.c2 * Y_m1)
                / (1 + UTILITIES_COLOUR.c3 * Y_m1)
            ),
            UTILITIES_COLOUR.m2,
        )

    def PqToLinear(pq_values: torch.Tensor):
        E_1_m1 = pq_values ** (1 / UTILITIES_COLOUR.m2)
        return 100.0 * torch.pow(
            (
                torch.clamp(E_1_m1 - UTILITIES_COLOUR.c1, min=1e-6)
                / (UTILITIES_COLOUR.c2 - UTILITIES_COLOUR.c3 * E_1_m1)
            ),
            (1 / UTILITIES_COLOUR.m1),
        )

    # https://en.wikipedia.org/wiki/SRGB

    def LinearToSrgb(x: torch.Tensor) -> torch.Tensor:
        return torch.where(
            x <= 0.0031308, 12.92 * x, 1.055 * torch.pow(x, 1 / 2.4) - 0.055
        )

    def SrgbToLinear(x: torch.Tensor) -> torch.Tensor:
        return torch.where(x <= 0.04045, x / 12.92, torch.pow((x + 0.055) / 1.055, 2.4))

    # https://en.wikipedia.org/wiki/Rec._709

    # def LinearToRec709(x: torch.Tensor) -> torch.Tensor:
    #     return torch.where(x < 0.018, 4.5 * x, 1.099 * torch.pow(x, 0.45) - 0.099)

    # def Rec709ToLinear(x: torch.Tensor) -> torch.Tensor:
    #     return torch.where(x < 0.081, x / 4.5, torch.pow((x + 0.099) / 1.099, 1 / 0.45))
